...
class Tournament:
    ...
    def addPlayerToTab(self,i_player):
        l = len(self.player_list)
        if l == 8:
            return
        if l == 0:
            i_player.id = 0
        else:
            i_player.id = self.player_list[l-1].id + 1    
    ...
    def initFirstRound(self):
        ...
        l = len(sorted_player_list)
            # creation des match
        self.initRounds()
        for index in range(int(l/2)):
            self.rounds[0].match_list.append(Match(sorted_player_list[index],sorted_player_list[int(l/2)+index]))
        ...
        for index in range(0,int(len(self.player_list)/2)):
            ...
            while index_player < len(self.scoreboard):
                if opponent.count(self.scoreboard[index_player].id) == 0:
                    ...
                    index_player = 100
                index_player += 1
            self.rounds[self.currRound-1].match_list.append(Match(player_1,player_2))
    ...
    def serialize(self):
        ...
        serialized_player_list = [
            player.serialize() for player in self.player_list
        ]

        serialized_rounds = [
            rounde.serialize() for rounde in self.rounds
        ]
        serialized_scoreboard = [
            player.serialize() for player in self.scoreboard
        ]
